...
import { facturasVentasClienteService } from '../services/facturas-ventas-cliente.service'

export const useFacturasVentasClienteStore = defineStore('facturasVentasCliente', () => {
  // Estado
  const facturas = ref([])
  ...
  async function fetchFacturasVenta() {
    ...
    error.value = null
    try {
      const data = await facturasVentasClienteService.getFacturasVenta()
  ...
  async function fetchSoportesPago(codigo) {
    try {
      const data = await facturasVentasClienteService.getSoportesPago(codigo)
  ...
  async function subirSoportePago(codigo, archivo) {
    ...
    error.value = null
    try {
      await facturasVentasClienteService.subirSoportePago(codigo, archivo)
      // Recargar soportes
  ...
  async function descargarSoporte(idSoporte) {
    try {
      const result = await facturasVentasClienteService.descargarSoporte(idSoporte)
