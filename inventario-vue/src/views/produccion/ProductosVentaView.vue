    ...
    <div class="view-container">
      ...
      <div class="page-header">
        <div class="header-left">
          ...
          <div>
            ...
            <p class="page-sub">Gestiona tu catálogo de productos disponibles para vender</p>
          </div>
        </div>
      ...
      <div class="kpi-grid">
        <div class="kpi-card">
          ...
          <div class="kpi-value">{{ store.totalProductosVenta }}</div>
          <div class="kpi-sub">{{ store.productosActivos.length }} activos</div>
        </div>
        <div class="kpi-card">
          <div class="kpi-label">GRUPOS DE PRODUCTOS</div>
          <div class="kpi-value">{{ store.totalGrupos }}</div>
          <div class="kpi-sub">Categorías creadas</div>
        </div>
      </div>
      ...
      <div class="filtros-bar">
        ...
        <v-select
          v-model="store.filtrosProductos.grupo"
          :items="['TODOS', ...store.grupoProductos.map(g => g.nombre)]"
          label="Grupo"
          variant="outlined"
          size="small"
          class="grupo-select"
      ...
      <div class="tabla-container">
        <div class="tabla-wrapper">
          <table class="data-table">
            ...
            <tbody>
              <tr v-if="store.productosVenta.length === 0">
              ...
              <tr v-for="prod in store.productosVenta" :key="prod.id" class="tabla-row">
                <td class="col-codigo">
                  <span class="codigo-badge">{{ prod.codigo || '-' }}</span>
                </td>
                <td class="col-nombre">{{ prod.nombre }}</td>
                <td class="col-grupo">{{ prod.grupo || '-' }}</td>
...
async function cargarProductos() {
  await store.fetchProductosVenta(store.filtrosProductos)
}

function abrirFormulario(producto = null) {
  alert('Formulario: ' + (producto ? 'Editar ' + producto.nombre : 'Crear nuevo producto para venta'))
}

async function eliminarProducto(id) {
  if (confirm('¿Eliminar este producto?')) {
    await store.eliminarProductoVenta(id)
  }
}

onMounted(async () => {
  await cargarProductos()
  await store.fetchGrupoProductos()
})
</script>
