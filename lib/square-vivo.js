...
function crearSquareVivo({ pool }) {
    ...
    function snapshot(empresa) {
        ...
        return {
            ...
            totales: {
                ...
                proyeccion: sedes.reduce((s, x) => s + (x.proyeccion || 0), 0),
    ...
    /** Horas que faltan para el cierre del servicio. La jornada cruza la
     *  medianoche (se cierra de madrugada), así que después de las 12 el
     *  cálculo tiene que seguir contando hacia adelante, no hacia atrás. */
    function horasHastaCierre(ahora) {
        const cierre = parseFloat(process.env.SQUARE_HORA_CIERRE || '3'); // 3am
        const h = horaLocal(ahora);
        // Antes de las 5am ya estamos en la madrugada del mismo día de operación
        const restante = h < 5 ? (cierre - h) : (24 - h + cierre);
        return Math.max(0, restante);
    }

    /** Ritmo de venta, proyección de cierre y agotamiento estimado.
     *
     *  La proyección es deliberadamente simple: lo vendido más el ritmo de la
     *  última hora por las horas que faltan. No intenta adivinar la curva del
     *  día -para eso haría falta histórico por hora, y la tabla `ventas` solo
     *  guarda totales diarios-. Se etiqueta como "a este ritmo" justamente
     *  para no venderla como una predicción. */
    function aplicarRitmo(e, porSede) {
        ...
        for (const s of porSede.values()) {
            ...
            if (!suyas.length) {
                s.ritmoHora = 0; s.proyeccion = null; s.horasRestantes = restante;
                continue;
            }
            ...
            s.proyeccion = s.ventas + s.ritmoHora * restante;
    ...
    async function recalcular(empresa) {
        ...
        try {
            ...
            for (const s of porSede.values()) {
                ...
                s.pctVsPromedio = (h && h.promedio > 0 && s.proyeccion != null)
                    ? ((s.proyeccion - h.promedio) / h.promedio) * 100
                    : null;
            }
        } catch (err) {
