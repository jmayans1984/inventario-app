...
function crearSquareVivo({ pool }) {
    ...
    // Umbral y ventana de la alerta de "nivel crítico de pedidos", por
    // empresa. Cache corto (no cero): recalcular() se llama en cada webhook
    // de Square, y sin cache cada orden nueva pegaría contra config_general.
    let alertaPorEmpresa = new Map(); // empresa -> { monto, minutos, cargadoEn }
    ...
    function snapshot(empresa) {
        ...
        const sedes = [...e.sedes.values()]
            .map(s => ({
                ...
                costoMP: s.costoMP,
                pctCosto: s.pctCosto,
                productosSinCosto: s.productosSinCosto,
                nombresSinCosto: s.nombresSinCosto,
                ...
                alertaCritica: s.alertaCritica || false,
                ventaVentanaAlerta: s.ventaVentanaAlerta || 0,
                alertaMinutos: s.alertaMinutos,
                alertaMonto: s.alertaMonto || 0,
            }))
        ...
        return {
            ...
            totales: {
                ...
                productosSinCosto: sedes.reduce((s, x) => s + (x.productosSinCosto || 0), 0),
                propinas: sedes.reduce((s, x) => s + (x.pagos?.propinas || 0), 0),
                promedioDia: sedes.reduce((s, x) => s + (x.promedioDia || 0), 0),
                // El menor de las sedes: si una tiene 3 semanas de historial y
                // otra 8, decir "8" sobrevende la base de la comparación.
                muestrasDia: (() => {
                    const m = sedes.reduce(
                        (acc, x) => (x.muestrasDia ? Math.min(acc, x.muestrasDia) : acc),
                        Infinity);
                    // Ojo: `Infinity || 0` devuelve Infinity, no 0 — es un
                    // valor verdadero. Sin este chequeo, cuando ninguna sede
                    // tiene historial se colaba un Infinity al JSON.
    ...
    /** Costo de materia prima de cada receta, por SKU.
     *
     *  Antes esto se calculaba expandiendo la receta en sus artículos de
     *  inventario y sumando el `precio_costo` de cada uno. Estaba mal: esa
     *  expansión solo incluye los insumos con control de inventario, así que
     *  dejaba fuera todo lo demás de la receta y daba un food cost del 21%
     *  cuando el real ronda el 30%.
     *
     *  `recetas.valor` ya trae el costo COMPLETO de la receta precalculado, y
     *  `recetas.codigo` es el mismo SKU que manda Square. Es una sola búsqueda
     *  y es el número correcto. Se cachea 10 minutos: los costos cambian de un
     *  día para otro, no dentro del servicio. */
    async function mapaCostoRecetas() {
        if (costos.mapa.size && Date.now() - costos.cargadoEn < 600000) return costos.mapa;
        const r = await pool.query(
            `SELECT TRIM(codigo::text) AS sku, COALESCE(valor, 0) AS costo FROM recetas`
        );
        const mapa = new Map();
        for (const row of r.rows) mapa.set(row.sku, parseFloat(row.costo) || 0);
    ...
    /** Ritmo de venta de la sede.
     *
     *  Aquí hubo dos cosas que se quitaron por la misma razón: una proyección
     *  de cierre (extrapolaba el ritmo de la hora pico y sobreestimaba hasta
     *  3x) y una estimación de a qué hora se agotaría cada insumo. Las dos
     *  suponían un ritmo constante que en la práctica no se sostiene, y un
     *  número que falla por mucho es peor que no mostrarlo. */
    ...
    /** Umbral y ventana de la alerta de "nivel crítico de pedidos", desde
     *  config_general. Ambos son parametrizables por Configuración; si el
     *  monto no está puesto, la alerta queda desactivada (nunca marca nada
     *  en rojo). Cache de 30s: recalcular() corre en cada webhook de Square. */
    async function configAlerta(empresa) {
        const cache = alertaPorEmpresa.get(empresa);
        if (cache && Date.now() - cache.cargadoEn < 30000) return cache;

        let monto = 0, minutos = 60;
        try {
            const r = await pool.query(
                `SELECT alerta_ventas_monto, alerta_ventas_minutos
                 FROM config_general WHERE empresa::text = $1`,
                [String(empresa)]
            );
            if (r.rows.length) {
                monto = parseFloat(r.rows[0].alerta_ventas_monto) || 0;
                minutos = parseInt(r.rows[0].alerta_ventas_minutos) || 60;
            }
        } catch (err) {
            console.error('[square-vivo] no se pudo leer la config de alerta:', err.message);
        }

        const cfg = { monto, minutos, cargadoEn: Date.now() };
        alertaPorEmpresa.set(empresa, cfg);
        return cfg;
    }

    /** "Nivel crítico de pedidos": cuánto vendió cada sede en los últimos N
     *  minutos (parametrizable), para saber si conviene seguir tomando
     *  pedidos por teléfono ahí. Se calcula sobre e.ordenes SIN truncar —
     *  igual que aplicarRitmo — porque listaOrdenes (la que viaja al
     *  navegador) recorta a las MAX_ORDENES_ENVIADAS más recientes, y en un
     *  punto muy ocupado esa ventana puede tener más pedidos que eso: contar
     *  sobre la lista recortada subestimaría justo en el caso que la alerta
     *  tiene que detectar. */
    async function aplicarAlertaCritica(empresa, e, porSede) {
        const cfg = await configAlerta(empresa);
        const activa = cfg.monto > 0;
        const desde = new Date(Date.now() - cfg.minutos * 60000);

        for (const s of porSede.values()) {
            if (!activa) {
                s.alertaCritica = false;
                s.ventaVentanaAlerta = 0;
                s.alertaMinutos = cfg.minutos;
                s.alertaMonto = 0;
                continue;
            }
            const ventaVentana = e.ordenes
                .filter(o => o.sedeCodigo === s.codigo && o.estado !== 'CANCELED' && new Date(o.creada) >= desde)
                .reduce((a, o) => a + (o.bruto ?? o.total), 0);
            s.ventaVentanaAlerta = ventaVentana;
            s.alertaMinutos = cfg.minutos;
            s.alertaMonto = cfg.monto;
            s.alertaCritica = ventaVentana >= cfg.monto;
        }
    }
    ...
    async function recalcular(empresa) {
        ...
        // Food cost del turno: se valoriza cada plato vendido con el costo de
        // su receta. Se cuenta sobre lo VENDIDO, no sobre el inventario
        // consumido — son cosas distintas y solo la primera tiene precio.
        try {
            const costoReceta = await mapaCostoRecetas();
            for (const s of porSede.values()) {
                let costoMP = 0;
                const sinCosto = [];
                for (const prod of s.productos.values()) {
                    const unit = prod.sku ? costoReceta.get(String(prod.sku).trim()) : undefined;
                    if (unit === undefined || unit <= 0) {
                        // Sin costo cargado, ese plato entra como cero y el food
                        // cost sale más bajo de lo real: hay que poder avisarlo.
                        if (prod.cantidad > 0) sinCosto.push(prod.nombre);
                        prod.costo = null;
                        continue;
                    }
                    prod.costoUnit = unit;
                    prod.costo = unit * prod.cantidad;
                    costoMP += prod.costo;
                }
                s.costoMP = costoMP;
                s.pctCosto = s.ventas > 0 ? (costoMP / s.ventas) * 100 : 0;
                s.productosSinCosto = sinCosto.length;
                s.nombresSinCosto = sinCosto.slice(0, 3);
            }
        } catch (err) {
            console.error('[square-vivo] no se pudo calcular el food cost:', err.message);
        }
        ...
        // Nivel crítico de pedidos: venta en la ventana configurada
        try {
            await aplicarAlertaCritica(empresa, e, porSede);
        } catch (err) {
            console.error('[square-vivo] no se pudo calcular la alerta crítica:', err.message);
        }
    ...
    // "Nivel crítico de pedidos" se apaga solo con el paso del tiempo, no
    // solo cuando entra una orden nueva: es una ventana de los últimos N
    // minutos, así que puede dejar de cumplirse sin que pase nada — la sede
    // se calma y ninguna orden nueva llega a disparar el recálculo. Este
    // reloj es aparte del de arriba a propósito: no le pega a la API de
    // Square (recalcular() solo trabaja sobre e.ordenes, ya en memoria, con
    // el food cost y el promedio semanal cacheados 10 min / 1 hora), así que
    // puede correr mucho más seguido sin sumar carga real.
    const relojAlerta = setInterval(() => {
        for (const [empresa, lista] of clientes) {
            if (!lista.size) continue;
            recalcular(empresa).then(() => emitir(empresa)).catch(() => {});
        }
    }, 30000);
    if (relojAlerta.unref) relojAlerta.unref();
