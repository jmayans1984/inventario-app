    ...
    resumenDiv.innerHTML = `
        <div class="resumen">
            <div class="resumen-item">
                ...
                <div class="resumen-value" style="color: #60a5fa;">${formatCurrency(resumen.saldo_inicial)}</div>
            </div>
            <div class="resumen-item">
                <div class="resumen-label">Total Ingresos</div>
                <div class="resumen-value" style="color: var(--success);">+${formatCurrency(resumen.total_ingresos)}</div>
            </div>
            <div class="resumen-item">
                <div class="resumen-label">Total Egresos</div>
                <div class="resumen-value" style="color: var(--danger);">-${formatCurrency(resumen.total_egresos)}</div>
            </div>
            <div class="resumen-item">
                <div class="resumen-label">Saldo Final</div>
                <div class="resumen-value" style="color: #a78bfa;">${formatCurrency(resumen.saldo_final)}</div>
            </div>
        </div>
    `;
    ...
    movimientosData.forEach((mov, index) => {
        ...
        htmlTabla += `
            <tr style="background: ${bgColor};">
                ...
                <td>${mov.concepto || '-'}</td>
                <td class="text-right monospace" style="color: var(--success);">${ingreso > 0 ? formatCurrency(ingreso) : ''}</td>
                <td class="text-right monospace" style="color: var(--danger);">${egreso > 0 ? formatCurrency(egreso) : ''}</td>
            </tr>
        `;
    });
